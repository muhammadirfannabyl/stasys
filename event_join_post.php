<?php
	...
	if (isset($_GET["no"])){
		...
		if(mysqli_num_rows($query0) > 0){
			echo '
			<script>
				alert("FAIL: You have already joined this event!");
			...
			if ($count['count'] > $rowevent['quota']){
				echo '
				<script>
					alert("FAIL: You cannot join this full event!");
					window.location = "home.php";
				</script>
				';
			}else{
				...
				if($result){
					echo '
					<script>
						alert("SUCCESS: Welcome to the event.");
						window.location = "event.php?no='.$rowevent["id"].'";
					</script>
					';
				}
				else{
					echo '
					<script>
						alert("FAIL: Cannot join this event due to server error.");
						window.location = "home.php";
					</script>
					';
				}
